...
import type { PageSnapshot, SnapshotElement } from './protocol.ts'

/** Element inventory script run inside the page. */
const COLLECT_SCRIPT = (): Array<Omit<SnapshotElement, 'index'>> => {
  const SELECTORS = [
  ...
  const NAME_CAP = 80
  const elements: Element[] = [...document.querySelectorAll<Element>(SELECTORS)]
  ...
  const stateFor = (element: Element): string | undefined => {
 ...
 */
export function diffSnapshot(previous: PageSnapshot, current: PageSnapshot): string {
  const lines: string[] = []
 ...
 */
export async function collectSnapshot(page: Page, maxItems: number): Promise<PageSnapshot> {
  ...
  const elements: SnapshotElement[] = visibleFirst.slice(0, maxItems).map((element, position) => ({
    ...element,
    index: position + 1,
  }))
  return {
