...
interface CssResult extends Picked {
  block: string
}
...
function mapRect(picked: Picked, frame: HTMLElement): { left: number; top: number; width: number; height: number } {
...
function insertIntoComposer(text: string): boolean {
  const textarea = document.querySelector<HTMLTextAreaElement>('textarea:not([aria-hidden="true"])')
...
function buildPanel(ctx: ClientContext, mode: 'dock' | 'tab'): {
  root: HTMLElement; image: HTMLImageElement; dot: HTMLElement; urlLine: HTMLElement; empty: HTMLElement
  stage: HTMLElement; frame: HTMLElement; overlay: HTMLElement; pickBtn: HTMLButtonElement
  card: HTMLElement; cardCode: HTMLElement; addBtn: HTMLButtonElement
  tabStrip: HTMLElement; openCloseBtn: HTMLButtonElement
  renderTabs: (tabs: ReadonlyArray<{ index: number; url: string; active: boolean }>) => void
  ...
  // 面板可见性 ⇔ `--dsh-browser-panel-width`：隐藏时变量必须归零，否则
  // `#root{margin-right:var(--dsh-browser-panel-width)}` 会把主内容区右侧
  // 挤掉整条面板宽度，造成"页面填不满"。
  ...
  // dock 模式初始隐藏（等 session 出现）；tab 模式可见性由宿主容器控制，
  // 初始必须可见（hidden=true 会让 startPolling 跳过抓帧 → 侧边栏画面空白）。
  ...
  const sessionOf = (): string | undefined => {
    ...
    return sessions?.list?.current
  }
  const tabAction = (action: 'new' | 'refresh' | 'close'): void => {
    const sessionId = sessionOf()
    if (sessionId === undefined) return
    void fetch(`${PANEL_ROUTE}/tab?session=${encodeURIComponent(sessionId)}&action=${action}`, { cache: 'no-store' }).catch(() => {})
  }
  ...
  openCloseBtn.addEventListener('click', () => {
    ...
    void fetch(`${PANEL_ROUTE}/${open ? 'close' : 'open'}?session=${encodeURIComponent(sessionId)}`, { cache: 'no-store' }).catch(() => {})
  })
  head.append(dot, title, openCloseBtn, newTabBtn, refreshBtn, closeTabBtn, pickBtn, close)

  // Tab 列表条：显示所有标签，点击切换，x 关闭。
  const tabStrip = document.createElement('div')
  tabStrip.className = 'bp-tabs'
  tabStrip.hidden = true
  const renderTabs = (tabs: ReadonlyArray<{ index: number; url: string; active: boolean }>): void => {
    ...
    for (const tab of tabs) {
      ...
      x.addEventListener('click', (event) => {
        ...
        const sessionId = sessionOf()
        if (sessionId === undefined) return
        void fetch(`${PANEL_ROUTE}/tab?session=${encodeURIComponent(sessionId)}&action=close&index=${tab.index}`, { cache: 'no-store' }).catch(() => {})
      })
      item.append(label, x)
      item.addEventListener('click', () => {
        ...
        const sessionId = sessionOf()
        if (sessionId === undefined) return
        void fetch(`${PANEL_ROUTE}/tab?session=${encodeURIComponent(sessionId)}&action=switch&index=${tab.index}`, { cache: 'no-store' }).catch(() => {})
      })
  ...
  dropBtn.addEventListener('click', () => { card.hidden = true; exitPick() })
  ...
  let picking = false
  let lastPick: CssResult | null = null
  ...
  const coordsOf = (event: MouseEvent): { x: number; y: number } | null => {
  ...
  stage.addEventListener('mousemove', (event) => {
    ...
    if (sessionId === undefined || coords === null) return
    void fetch(`${PANEL_ROUTE}/pick?session=${encodeURIComponent(sessionId)}&x=${coords.x}&y=${coords.y}`, { cache: 'no-store' })
      .then(response => response.ok ? response.json() as Promise<Picked> : null)
      .then(picked => {
        if (picked === null || !picking) { overlay.style.display = 'none'; return }
  ...
  stage.addEventListener('click', (event) => {
    ...
    const coords = coordsOf(event)
    if (sessionId === undefined || coords === null) return
    void fetch(`${PANEL_ROUTE}/css?session=${encodeURIComponent(sessionId)}&x=${coords.x}&y=${coords.y}`, { cache: 'no-store' })
      .then(response => response.ok ? response.json() as Promise<CssResult> : null)
      .then(result => {
  ...
  addBtn.addEventListener('click', () => {
    ...
    const inserted = insertIntoComposer(`\`\`\`css\n${lastPick.block}\n\`\`\``)
    if (inserted) {
      addBtn.textContent = '✅ 已添加'
      setTimeout(() => { addBtn.textContent = '➕ 添加到对话' }, 1500)
    }
  })

  return { root, image, dot, urlLine, empty, stage, frame, overlay, pickBtn, card, cardCode, addBtn, tabStrip, openCloseBtn, renderTabs, setPanelVisible }
}
...
function startPolling(
  ...
  const tick = async (): Promise<void> => {
    if (stopped) return
    const sessions = ctx.get('sessions') as { list?: SessionList } | undefined
    const sessionId = sessions?.list?.current
    if (sessionId === undefined || panel.root.hidden) {
    ...
    try {
      ...
      const stateResponse = await fetch(`${PANEL_ROUTE}?session=${encodeURIComponent(sessionId)}`, { cache: 'no-store' })
      const state = await stateResponse.json() as {
        open: boolean; url?: string; title?: string
        tabs?: ReadonlyArray<{ index: number; url: string; active: boolean }>
      }
...
export function apply(ctx: ClientContext): void {
  ...
  const betterSidebar = ctx.get('betterSidebar') as { registerTab?: (descriptor: unknown) => () => void } | undefined
  let panelRef: ReturnType<typeof buildPanel> | null = null
  let stopPoll: (() => void) | null = null
  let mounted = false

  if (betterSidebar?.registerTab) {
    // 强兼模式：注册侧边栏 tab，打开时懒挂载面板，关闭时销毁。
    // visible 变 true（用户点开 tab）时自动 open 浏览器——模型无需先调
    // browser_open，画面即刻同步。
    betterSidebar.registerTab({
      id: 'browser:web',
      title: '浏览器',
      icon: (size: number) => React.createElement('span', { style: { fontSize: size } }, '🌐'),
      single: true,
      component: (props: { visible?: boolean; scope?: { sessionId?: string } }) => {
        const sessionId = props.scope?.sessionId ?? ''
        const autoOpened = React.useRef(false)
        React.useEffect(() => {
          if (props.visible && !autoOpened.current && sessionId !== '') {
            autoOpened.current = true
            void fetch(`${PANEL_ROUTE}/open?session=${encodeURIComponent(sessionId)}`, { cache: 'no-store' }).catch(() => {})
          }
        }, [props.visible])
  ...
  // ctx.effect 的 callback 立即执行（返回的才是卸载 cleanup）——挂载/轮询
  // 放这里，停止轮询/移除面板放返回的 cleanup，避免面板被立即卸载。
  ctx.effect(() => {
    return () => {
      stopPoll?.()
      stopSessions()
