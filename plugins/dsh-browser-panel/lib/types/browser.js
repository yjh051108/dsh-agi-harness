...
export class BrowserSession {
    ...
    async respondDialog(id, accept, text) {
        ...
        if (dialog === undefined)
            throw new Error(`dialog ${id} not pending`);
    ...
    async open(profileDir) {
        ...
        for (const [key, value] of Object.entries(process.env)) {
            if (/^(http|https|all|no)_proxy$/i.test(key))
                continue;
            if (value !== undefined)
                cleanEnv[key] = value;
        }
    ...
    wirePage(page) {
        page.on('console', (message) => {
            ...
            if (this.consoleBuffer.length > BrowserSession.CONSOLE_CAP)
                this.consoleBuffer.splice(0, this.consoleBuffer.length - BrowserSession.CONSOLE_CAP);
        });
