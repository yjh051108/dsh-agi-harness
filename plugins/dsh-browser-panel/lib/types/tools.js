 ...
 */
import { BrowserSession, PROFILE_ROOT } from "./browser.js";
import { collectSnapshot, diffSnapshot } from "./snapshot.js";
import { renderSnapshot } from "./protocol.js";
import { click, press, scroll, typeText, waitStable } from "./actions.js";
import { describeFrame } from "./vision.js";
...
export class BrowserSessionRegistry {
    ...
    async closeIdle(idleMs) {
        ...
        const candidates = [];
        for (const session of this.sessions.values()) {
            if (session.isOpen && session.idleMs() > idleMs)
 ...
 */
export function registerBrowserTools(ctx, registry, options) {
    for (const tool of defineTools(registry, options))
...
function defineTools(registry, options) {
    ...
    const sessionFor = async (exec) => {
        ...
        if (!session.isOpen)
            await session.open(profileDirFor(sessionId));
        session.touch();
        return session;
    };
    ...
    return [
        ...
        },
        {
            ...
            async execute(args, exec) {
                const session = await sessionFor(exec);
                const value = await session.page.evaluate(args.expression);
        ...
        ...['back', 'forward', 'reload'].map((action) => ({
            ...
            async execute(_args, exec) {
                ...
                if (action === 'back')
                    await page.goBack({ waitUntil: 'load', timeout: 30_000 }).catch(() => null);
                else if (action === 'forward')
                    await page.goForward({ waitUntil: 'load', timeout: 30_000 }).catch(() => null);
                else
        ...
        },
        {
            ...
            async execute(args, exec) {
                ...
                if (!states.includes(state))
                    return { text: `Invalid state "${state}"; expected ${states.join(' | ')}.` };
                try {
                    await session.page.waitForSelector(selector, { state: state, timeout: timeoutMs });
        ...
        },
        {
            ...
            async execute(args, exec) {
                ...
                if (entries.length === 0)
                    return { text: 'No console messages recorded.' };
        ...
        },
        {
            ...
            async execute(_args, exec) {
                ...
                if (dialogs.length === 0)
                    return { text: 'No pending dialogs.' };
        ...
        },
        {
            ...
            async execute(args, exec) {
                ...
                if (downloads.length === 0)
                    return { text: 'No downloads recorded.' };
