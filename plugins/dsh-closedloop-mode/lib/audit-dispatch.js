...
/** 审材切片（v0.6.32 任务语标签版）：目标步预测与依据 + 邻域 + 本单完成条件。禁全栈。
 *  工单契约只住模板一套（audit-rotation）——审材不再自带输出契约（案底：双契约两代拼接）。 */
export function auditBrief({ stackRaw, targetTitle, cost, stackPath = '' }) {
  ...
  const V = { improve: '改善', maintain: '保持', dip: '暂退' }
  const lines = [
    `【审材·机械切片】目标 #${t.n}「${cut(t.title, 28)}」（已闭合）`,
    `预测：${(t.predictions || []).map((p) => `${p.key}=${cut(p.value, 18)}（依据：${cut(p.source, 22)}）`).join('｜')}`,
    `代价：${(t.cost || []).map((c) => `${cut(c.failure, 20)}（${cut(c.weight, 12)}）`).join('｜')}`,
    `规则 ${(t.law || []).length} 条·通道 ${(t.measure?.channels || []).length} 个·预期 ${V[t.vExpect] || '?'}·档位 ${t.dv ? `${t.dv.before}→${t.dv.after}` : '?'}·吻合 ${(t.agreed || []).length}·不吻合 ${(t.discrepancies || []).length}`,
    nb.length ? `相邻步：${nb.join(' ⇄ ')}` : '相邻步：无（单步）',
    cost?.assertions?.length ? `本单完成条件：${cost.assertions.map((a) => cut(a.text, 24)).join('｜')}` : '本单完成条件：（盘档无）',
    stackPath ? `盘档原文：${stackPath}（引文 quotes 须为该文件逐字子串，含中文与转义原样——改读此路径，勿另找副本）` : '',
  ].filter(Boolean)
...
export function parseVerdict({ stackRaw, verdictText }) {
  ...
  // v0.8.6 引文规范化（案底 4237f058：quotes 元素为对象 → String(q)='[object Object]' →
  // 逐字校验必然失败，被误诉为「伪造」）：字符串原样逐字；对象先串化再比对，
  // 拒因明确指路（对象必不符，quotes 须为字符串数组）。
  const normQ = (q) => (typeof q === 'string' ? q : JSON.stringify(q))
  const bad = (Array.isArray(v.quotes) ? v.quotes : []).filter((q) => !raw.includes(normQ(q)))
  const hasObjectQuote = (Array.isArray(v.quotes) ? v.quotes : []).some((q) => typeof q !== 'string')
  if (bad.length) errors.push(`引文非盘档原文（伪造/改写即拒）: ${bad.map((q) => cut(normQ(q), 16)).join('；')}${hasObjectQuote ? '——注意：quotes 元素是对象，已被串化比对必不符（quotes 须为字符串数组，取盘档逐字节原文）' : ''}`)
...
/** 审材切片直接落盘（读盘通道内联在 ensureSeed/buildBrief——r49 扫尸案：原 readStackRaw 导出零引用已删，git 历史留设计初衷）。 */
