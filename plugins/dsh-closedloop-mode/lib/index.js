    ...
    if (sid !== undefined) {
      ...
      if (st.stage === 'weights' || st.stage === 'rolling' || st.stage === 'final') {
        for (let i = (messages || []).length - 1; i >= 0; i--) {
          ...
          const t = txt2.trim()
          const full = t ? scanIntentFull(t) : { intent: null, source: null }
          const intent = full.intent
          // scanLog：扫描看见了什么落盘可查（v0.3.1④——「继续」不翻转这类问题从猜测变成读账；
          // v0.8.13 补 via=意图来源：json 信封 / text 散文 / json-invalid 非法信封 / too-long 超长不猜）
          setState(sid, { ...state(sid), scanLog: { at: Date.now(), len: t.length, intent, via: full.source, stage: st.stage, head: t.slice(0, 24) } })
