...
export function installEngramTools(ctx, relay) {
    ...
    disposers.push(ctx.tools.register(defineTool({
        ...
        execute: async (args, exec) => {
            ...
            const parts = [];
            if (revived)
                parts.push('🗄 该记忆此前已退役——本次展开已将其复活（重新参与召回）');
    ...
    disposers.push(ctx.tools.register(defineTool({
        ...
        execute: async (args, exec) => {
            ...
            const viewer = viewerOf(exec);
            if (!isVisible(node, viewer))
                return `无权退役 [[${node.title}]]（${node.layer} 层对当前会话不可见）`;
            if (node.status === 'pending')
                return `[[${node.title}]] 是待确认（⏳）节点，尚未生效——无需退役；不要可直接 engram_reject`;
            if (node.status === 'retired')
                return `[[${node.title}]] 已是退役状态（🗄）`;
    ...
    disposers.push(ctx.tools.register(defineTool({
        ...
        execute: async (args) => {
            ...
            const crossSee = async () => {
                try {
                    const w = await relay.recall(claim, 2);
                    const es = (w?.engrams || []).filter((e) => e && e.title);
                    return es.length ? `\n📎 相关记忆：${es.map((e) => `[[${e.title}]]`).join('、')}（engram_open 展开）` : '';
                }
                catch {
                    return '\n📎 相关记忆：检索通道不可用（不装见过）';
                }
            ...
            if (m.status === 'error')
                return `（灵枢验证不可用：${m.note ?? ''}）${await crossSee()}`;
            if (m.status === 'anchored')
                return `✓已锚定（${m.note ?? '图谱确认'}）：该主张与灵枢知识图谱一致，可溯源${await crossSee()}`;
            if (m.status === 'partial')
                return `~部分锚定：${m.note ?? '部分主张命中图谱'}${await crossSee()}`;
