...
## v0.3.21 — 判据不许空转：可证伪门 + 人判欠据门

> 起因是两份导出会话的复盘：交付物被判「非常垃圾」，而闭环报「11 条断言全绿·交付完成」。
> 查下来不是模型偷懒，是**判据本身是空卷**——模型自己出题、自己批卷，而唯一能判质量的通道
> （眼睛/耳朵/GPU）在那台机器上根本不存在。这一版补两道门，都不点具体判据的名字。

### 新增：判据可证伪门（`src/falsify.js`）

- **不变量**：判据必须能对「假货」说不。把被审者产物的内容替换成负对照再跑判据，判据若照样绿 ⇒ 该判据是空的 ⇒ 不许进合同、不许落账。
- **负对照三态（同字节尺寸，防体积门造成的假敏感）**：掏空壳（零字节）/ 字面壳（判据源码里的全部字符串字面量拼成）/ 空转壳（结构合法但什么都不做）。
- **锚定壳（第四态，实测逼出来的）**：字面壳会把判据源码里的**反向**字面量也拼进去（`check.mjs` 里的 `includes('TODO')`），反而让判据变红、被误判成「敏感」。锚定壳只取「真实产物里本来就出现的判据字面量」= 判据的正向要求集。
- **产物集 = 判据可达文件集**（本地 import 闭包 ≤3 层 + 源码里的路径字面量，限工作区内）∪ 本会话写入面——「判据依赖谁就伪造谁」，而不是「本会话写过谁」。（这条是自测抓出来的：门一开始对自包含判据假阳性。）
- **接线**：`decompose` 合同期早警（vacuous 当场拒收）+ `trySettleGroups` 落账门（先证尺子有刻度，再看读数）。变异期间连 **mtime 一起还原**（否则账本快检误判 stale，且进程被杀会留假货）。

### 新增：人判欠据门（`recordIOU/payIOU/openIOU` + `terminalCheck`）

- **口径换血**：`人判:` 判据从「挂账可审计」改成「**欠据**」——落账时登记，未付欠据一律阻止终端归零。
- **支付通道**：真人帧「签收 <组名>」/「签收全部」（帧判类同 r64 硬签收门，模型自写不算）。
- **终检回执**：未付时出 `⏸ 未归零：N 条人判欠据待开发者签收（机器部分已齐）`，逐条列出 + 给支付方式；不再出现「✅ 终端归零」。
- 语义边界：欠据**不阻断推进**，只阻断「完成」口径；纯 `cmd:` 组不产生欠据（机器有读数的项不许转人判）。

### 验证（本机实跑）

- 真实案卷重放：池核案（`session-1f8c4faa`）33 组文本 grep 判据 → **判 vacuous**；7 条人判项 → **未归零**（`scripts/check-falsify.mjs` / `scripts/check-iou.mjs` 双 PASS）。
- 全量 **509/509**；变异审计 **12 目标 · 88 变异 · 88 杀死 · 0 幸存 · 10 等价 · 杀死率 1**（新代码入靶 + 8 个手工变异点，因自动算子每类只取首命中位）。
- 发布副本自跑：6 个新测试文件 **44/44** + 两个回归脚本 PASS。
- 装机实测：构建 → 热重载（清 33 模块缓存、重建 fiber）→ 终检按新口径报「⏸ 未归零：1 条」→ 开发者「签收全部」→ 归零 ✅（这道门的第一次真实生效）。
