...
**依据**：杀死率只统计「测试是否变红」，不区分「红是因为断言了行为」还是「红是因为断言了实现细节」。**地基已具备**（v0.3.13）：账本现有 `killedBy` 与 `singleKiller`——实测 50 条变异中 **35 条只被一个测试文件杀死**，守护普遍只有一道。

**做法**（候选）：对单杀者优先补第二道断言（跨测试文件的交叉守护）；再考虑标记「只断言 truthy/长度/不抛错」的弱断言，比例进账本。
